...
check_question(
  ...
  button_label = "Submit answer", 
  q_id = 14,
  right = c("That's correct! The code: `aes(x = displ, y = hwy)` is mapping the data columns `displ` to the `x-axis` and `hwy` to the `y-axis`.")
)
```
:::
...
- the `geom_` layer, e.g, `geom_smooth(mapping = aes(x, y))`
...
Run `ggplot(data = mpg, aes(cty, hwy))`. What do you see?
...
**Which code produces the expected scatterplot? **

```{r}
#| label: placing aes in scatterplot
#| echo: FALSE
check_question(
  answer = c(
...
Copy the code in exercise 2 and add a `geom_smooth()` layer to the code.
...
**Where can we place the `mapping` argument?**

```{r}
#| label: placing aes with geom_smooth
#| echo: FALSE
check_question(
  answer = c(
  ...
  options = c(
   "The `mapping` argument should go in the `ggplot()`.",
   "The `mapping` argument should go in the `geom_point()`.", 
   "The `mapping` argument should go in the `geom_smooth()`.", 
   "The `mapping` argument should go into `geom_point()` AND  `geom_smooth()`."
  ), 
  type = "checkbox", 
